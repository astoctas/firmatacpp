...
#define FIRMATA_MSB2(x)					(((x) >> 14) & 0x7F)
 ...
 #define FIRMATA_STEPPER_ENABLE_PINS		0x0a
...
namespace firmata {
	...
	public:
		...
		void configStepper(uint8_t deviceNum, uint8_t type, uint8_t stepSize, uint8_t dirOrMotor1Pin, uint8_t dirOrMotor2Pin, uint8_t motorPin3 = 0, uint8_t motorPin4 = 0, uint8_t enablePin = 0);
		...
		void enablePinsStepper(uint8_t deviceNum, uint8_t enable1, uint8_t enable2);
	...
	private:
		uint8_t status[1024];
		uint8_t * encodeCustomFloat(double input);
		uint8_t* encode32BitSignedInteger(int32_t data);
	};
